    ...
    if winners:
        ...
        for d in winners:
            ...
            ax.annotate(
                ...
                fontsize=7,
                ha="center",
                color=family_color.get(family, "black"),
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Self-improvement progress visualization"
    )
    parser.add_argument("--data", help="Path to raw_data.json")
    parser.add_argument("--output", help="Path to output image")
    parser.add_argument(
        "--tracking-dir",
        help="Path to tracking/ directory (auto-discovers data and output)",
    )
    ...
    if args.tracking_dir:
        ...
        print(
            "Usage: plot_progress.py --tracking-dir /path/ OR --data /path/raw_data.json --output /path/progress.png"
        )
        sys.exit(1)
