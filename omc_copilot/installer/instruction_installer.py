...
from omc_copilot.adapters.runtime_factory import includes_copilot
...
class InstructionInstaller:
    ...
    def install(self, target_root: Path, runtime_name: str = "copilot") -> list[Path]:
        installed: list[Path] = []

        agents_template = (self.templates_root / "AGENTS.md").read_text(
            encoding="utf-8"
        )
        ...
        if includes_copilot(runtime_name):
            ci_template = (self.templates_root / "copilot-instructions.md").read_text(
                encoding="utf-8"
            )
            ...
            path_template = (
                self.templates_root / "instructions" / "omc.instructions.md"
